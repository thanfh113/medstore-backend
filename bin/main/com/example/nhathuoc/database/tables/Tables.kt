...
// Giấy tờ & Chứng nhận sản phẩm (số ĐK lưu hành, CoA, giấy phép nhập khẩu...)
object ProductCertificatesTable : Table("product_certificates") {
    val id         = varchar("id", 36)
    val productId  = varchar("product_id", 36).references(ProductsTable.id)
    // REGISTRATION | IMPORT_LICENSE | COA | GMP | OTHER
    val type       = varchar("type", 30).default("REGISTRATION")
    val name       = varchar("name", 200)          // Tên giấy tờ, số hiệu
    val fileUrl    = text("file_url")              // URL ảnh hoặc PDF
    val issuedBy   = varchar("issued_by", 200).nullable()   // Cơ quan cấp
    val issuedAt   = varchar("issued_at", 20).nullable()    // Ngày cấp (yyyy-MM-dd)
    val expiresAt  = varchar("expires_at", 20).nullable()   // Ngày hết hạn
    val createdAt  = datetime("created_at").defaultExpression(CurrentDateTime)
    override val primaryKey = PrimaryKey(id)
}
